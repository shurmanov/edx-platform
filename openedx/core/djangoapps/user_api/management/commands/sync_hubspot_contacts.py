...
class Command(BaseCommand):
    ...
    def _get_last_synced_contact_email(self, site_conf):
        ...
        try:
            ...
            if 'contacts' in response and len(response['contacts']) > 0:
                last_contact_email = response['contacts'][0].get('properties').get('email').get('value')
    ...
    def _get_batched_users(self, users, batch_size=2):
        """
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            for site_conf in hubspot_sites:
                ...
                for start, end, total, users_batch in self._get_batched_users(site_unsynced_users):
                    self.stdout.write(
                    ...
                    self.stdout.write(
                        'Succesfully synced users batch from {start} to {end} of {total} for site {site}'.format(
                            start=start, end=end, total=total, site=site_domain
                        )
                    )
